...
import test.main.android.rabbit.IRabbitHookLoadPackage;
import test.main.android.rabbit.IRabbitHookZygoteInit;
import test.main.android.rabbit.RabbitBridge;
import test.main.android.rabbit.XC_MethodHook;
import test.main.android.rabbit.callbacks.XC_LoadPackage;

import static test.main.android.rabbit.RabbitHelpers.findClass;


public class XPrivacy implements IRabbitHookLoadPackage, IRabbitHookZygoteInit {
	...
	public void initZygote(IRabbitHookZygoteInit.StartupParam startupParam) throws Throwable {
		...
		try {
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
				...
				RabbitBridge.hookAllMethods(at, "systemMain", new XC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param) throws Throwable {
						try {
							final ClassLoader loader = Thread.currentThread().getContextClassLoader();
							Class<?> am = Class.forName("com.android.server.am.ActivityManagerService", false, loader);
							RabbitBridge.hookAllConstructors(am, new XC_MethodHook() {
								@Override
								protected void afterHookedMethod(MethodHookParam param) throws Throwable {
									try {
				...
				RabbitBridge.hookMethod(mMain, new XC_MethodHook() {
					@Override
		...
		try {
			...
			for (Member member : listMember)
				try {
					...
					RabbitBridge.hookMethod(member, new XMethodHook(hook));
